...
export const SERVER_INSTRUCTIONS = [
	...
	"",
	"Начинайте с env_status: он покажет активный профиль запуска, файл настроек, версию vanessa-runner и строку подключения к ИБ. От окружения зависит, куда попадут изменения.",
	"",
	"В окне VS Code может быть несколько проектов 1С (каталогов с packagedef), инструменты без projectPath выполняются в текущем. project_list возвращает проекты окна и текущий проект, project_select с параметром root делает проект текущим, в том числе для пользователя. project_init создаёт packagedef в каталоге из обязательного projectPath (папка рабочей области или каталог из candidates ответа project_list) и делает этот проект текущим. У остальных инструментов projectPath выполняет один вызов в указанном проекте и текущий проект не меняет; относительный путь считается от текущего проекта.",
	"",
