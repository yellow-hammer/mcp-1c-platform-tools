...
### Добавление MCP вручную

В **VS Code** при установке расширения из VSIX MCP регистрируется автоматически. В **Cursor** — нужен ручной конфиг. Создайте в проекте 1С: `<проект_1С>/.cursor/mcp.json`. Путь в `args` задаётся **относительно корня открытого проекта**.
...
Перед релизом (при смене версии) — см. чек-лист в [шаблоне PR](.github/PULL_REQUEST_TEMPLATE.md).
